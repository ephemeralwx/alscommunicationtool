...
from tkinter import ttk, Button, Label, Entry, StringVar, END, Listbox, Scrollbar, Toplevel, Checkbutton, IntVar, Radiobutton, filedialog  # Changed to ttk
...
from elevenlabs import ElevenLabs, play  # Added ElevenLabs import
...
APP_PASSWORD = "tbrd nyny iygf eqox"
...
def trigger_scheduled_question(sq):
    ...
    speak("Alert, time for scheduled question.")  # Replaced pyttsx3 with speak()
    speak(sq.question)  # Replaced pyttsx3 with speak()
...
client_eleven = ElevenLabs(api_key="sk_4df4a51b6c514954e66c4e0b829acccf1574e6fb2a4aee25")

cloned_voice = None  # Global variable to store cloned voice
voice_status_var = None  # To be initialized in GUI
...
def speak(text):
    ...
    if cloned_voice is None:
        print("[ERROR] Voice not cloned yet. Please upload a voice recording.")
        return
    def tts():
        try:
...
client = OpenAI(api_key="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")  # Preserved original client initialization
...
def open_schedule_window():
    ...
    Label(schedule_window, text="Question:", font=("Helvetica", 10)).grid(row=0, column=0, padx=10, pady=5, sticky='e')
    question_var = StringVar()
    ttk.Entry(schedule_window, textvariable=question_var, width=50).grid(row=0, column=1, padx=10, pady=5)

    # Schedule Type
    Label(schedule_window, text="Schedule Type:", font=("Helvetica", 10)).grid(row=1, column=0, padx=10, pady=5, sticky='e')
    schedule_type_var = StringVar(value="time")
    ttk.Radiobutton(schedule_window, text="Specific Time", variable=schedule_type_var, value="time").grid(row=1, column=1, padx=10, pady=5, sticky='w')
    ttk.Radiobutton(schedule_window, text="Countdown Timer (seconds)", variable=schedule_type_var, value="timer").grid(row=2, column=1, padx=10, pady=5, sticky='w')
    ...
    Label(schedule_window, text="Hour (0-23):", font=("Helvetica", 10)).grid(row=3, column=0, padx=10, pady=5, sticky='e')
    time_entry = ttk.Entry(schedule_window, textvariable=time_var, width=10)
    time_entry.grid(row=3, column=1, padx=10, pady=5, sticky='w')
    # Remove initially if not needed
    timer_entry = ttk.Entry(schedule_window, textvariable=timer_var, width=10)
    timer_entry.grid(row=3, column=1, padx=10, pady=5, sticky='w')
    timer_entry.grid_remove()

    # Recurring Daily
    recurring_var = IntVar()
    ttk.Checkbutton(schedule_window, text="Repeat Daily", variable=recurring_var).grid(row=4, column=1, padx=10, pady=5, sticky='w')
    ...
    ttk.Button(schedule_window, text="Add Scheduled Question", command=add_scheduled_question).grid(row=5, column=1, padx=10, pady=10, sticky='e')

def upload_voice():
    """
    Allows the user to upload a voice recording and clones it using ElevenLabs.
    """
    global cloned_voice
...
def schedule_gui():
    """
    Creates the main GUI for scheduling questions, uploading voice, and setting emergency email.
    """
    global root, voice_status_var
    root = tk.Tk()
    root.title("Eye Gaze Tracker Scheduler")
    root.geometry("600x400")  # Set a fixed window size for consistency
    ...
    # Schedule a Question Section
    ttk.Button(root, text="Schedule a Question", command=open_schedule_window).pack(padx=20, pady=10)

    # ==== EMERGENCY EMAIL FEATURE START ====
    # Emergency Email Section
    Label(root, text="Emergency Email:", font=("Helvetica", 10)).pack(padx=10, pady=5)
    email_var = StringVar()
    email_entry = ttk.Entry(root, textvariable=email_var, width=50)
    email_entry.pack(padx=10, pady=5)
    ttk.Button(root, text="Set Emergency Email", command=lambda: set_emergency_email(email_var, email_entry)).pack(padx=10, pady=5)
    # ==== EMERGENCY EMAIL FEATURE END ====

    # ==== Upload Voice Recording SECTION ====
    Label(root, text="Upload Voice Recording:", font=("Helvetica", 10)).pack(padx=10, pady=5)
    ttk.Button(root, text="Upload Voice", command=upload_voice).pack(padx=10, pady=5)
    voice_status_var = StringVar(value="Voice Status: Not Cloned")
    Label(root, textvariable=voice_status_var, font=("Helvetica", 10, "italic")).pack(padx=10, pady=5)
    # ==== Upload Voice Recording SECTION END ====

    # Display Scheduled Questions
    Label(root, text="Scheduled Questions:", font=("Helvetica", 10)).pack(padx=10, pady=5)
    scheduled_listbox = Listbox(root, selectmode=tk.SINGLE, width=80, height=10)
    ...
    ttk.Button(root, text="Remove Selected Question", command=remove_selected_question).pack(padx=10, pady=5)
...
while cap.isOpened():
    ...
    # Draw bounding boxes for each region using colored outlines
                            ...
                            if direction_timer["up"] >= AGREEMENT_THRESHOLD:
                                if len(arrayOfResponses) > 0:
                                    ...
                                    try:
                                        ...
                                        speak(assistant_reply)  # Replaced pyttsx3 with speak()
                                        ...
                                        if scheduled_question_active:
                                            ...
                                            print(f"Analysis Prompt being sent to ChatGPT: {analysis_prompt}")  # Debug
                                            ...
                                            try:
                                                ...
                                                if analysis_reply == "true" or analysis_reply == "True":
                                                    send_email_for_help(emergency_email)
                                                elif analysis_reply == "false" or analysis_reply == "False":
                                                    pass  # Do nothing
                                                else:
                                                    print("[WARNING] Unexpected response from ChatGPT for analysis.")
                                            except Exception as e:
