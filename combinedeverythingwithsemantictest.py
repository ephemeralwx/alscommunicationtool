...
def trigger_scheduled_question(sq):
    ...
    # We'll get top 4 responses using get_top_responses (cosine similarity + GPT approach).
    # This merges the new semantic approach in place of the old looped GPT calls.
    ...
    try:
        for _ in range(num_responses):
            prompt = (
                ...
                f"Do not answer with any of these responses: {pastResponses}. Provide a short, single-sentence answer. Notice the past responses and try to make your answer different than the others. Pretend a user has to choose 2 different answers so your answers must be diverse and different, preferably opposite."
            )
...
while cap.isOpened():
    ...
    if key == ord('q'):
        ...
        break
        # The code does not explicitly train a gaze algorithm machine learning model.
        # Instead, it uses pre-trained models from the GazeTracking library and Mediapipe for face and eye tracking.
        # The calibration process maps eye positions to screen coordinates using linear regression.
        # Linear regression is trained in the map_gaze_to_screen function using np.linalg.lstsq.
