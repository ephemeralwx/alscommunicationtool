...
def open_schedule_window():
    ...
    def add_scheduled_question():
        ...
        if not question:
            ...
            engine = pyttsx3.init()
            engine.say("Question cannot be empty.")
            engine.runAndWait()
            return
        ...
        if schedule_type == "time":
            try:
                ...
                engine = pyttsx3.init()
                engine.say("Invalid time format.")
                engine.runAndWait()
                return
        else:
            try:
                ...
                engine = pyttsx3.init()
                engine.say("Invalid countdown timer format.")
                engine.runAndWait()
                return
        ...
        engine = pyttsx3.init()
        engine.say("Scheduled question added successfully.")
        engine.runAndWait()
...
def schedule_gui():
    ...
    def remove_selected_question():
        ...
        if selected:
            ...
            engine = pyttsx3.init()
            engine.say("Scheduled question removed successfully.")
            engine.runAndWait()
...
def set_emergency_email(email_var, email_entry):
    ...
    if email:
        ...
        if "@" in email and "." in email:
            ...
            engine = pyttsx3.init()
            engine.say("Emergency email set successfully.")
            engine.runAndWait()
            email_entry.delete(0, END)
        else:
            print("[ERROR] Invalid email format.")
            engine = pyttsx3.init()
            engine.say("Please enter a valid email address.")
            engine.runAndWait()
    else:
        print("[ERROR] Email field is empty.")
        engine = pyttsx3.init()
        engine.say("Please enter an email address.")
        engine.runAndWait()
# ==== EMERGENCY EMAIL FEATURE END ==== 
