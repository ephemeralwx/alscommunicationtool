...
# -------------------- NEW IMPORTS FOR SEMANTIC LEARNING FEATURE -------------------- #
import sqlite3 # For database operations
from sentence_transformers import SentenceTransformer
from sklearn.metrics.pairwise import cosine_similarity
# ----------------------------------------------------------------------------------- #

# ================== NEW IMPORTS FOR MULTIPLE ML MODELS ================== #
...
# Download the Whisper model at the beginning
def preload_whisper_model():
    print("[INFO] Downloading Whisper model...")
    try:
        whisper.load_model("tiny") # You can change "tiny" to "base", "small", etc.
...
def trigger_scheduled_question(sq):
    ...
    # ---------------- NEW CODE: Use our semantic method to generate 4 responses ----------------
    answers, sims, methods = get_top_responses(context, top_n=4, similarity_threshold=0.55)
    if len(answers) < 4:
        # fallback if for any reason we don't have 4
        print("[WARNING] Fallback: Not enough answers found from semantic method, filling with placeholders.")
        while len(answers) < 4:
            answers.append("N/A")

    # Put them into arrayOfResponses
    unique_answers = list(set(answers)) # Remove duplicates
    while len(unique_answers) < 4:
        unique_answers.append("N/A") # Fill in placeholders if needed
    arrayOfResponses.extend(unique_answers)
...
# ----------------------------------
# Initialize OpenAI & the GPT model
# ----------------------------------
client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
fine_tuned_client = OpenAI(api_key=os.getenv("OPENAI_FINE_TUNED_API_KEY"))
model = "gpt-4o-mini" # The original model used

# ------------------------ NEW: SEMANTIC LEARNING SETUP ------------------------ #
# Connect to the SQLite database (or create if doesn't exist)
...
def add_qa_to_db(question, answer):
    try:
        ...
        conn.commit()
        print(f"[INFO] Added Q&A to DB -> Q: {question}, A: {answer}")
    except Exception as e:
        print(f"[ERROR] Could not add Q&A to DB: {e}")

# For debugging, display all Q&A
def display_all_qa():
    cursor.execute('SELECT * FROM qa')
    rows = cursor.fetchall()
    print("\nCurrent Q&A in Database:")
    for row in rows:
        print(f'ID: {row[0]}')
...
if not sample_data:
    ...
    sample_data = fetch_all_qa()
    print("[INFO] Added default Q&A to the database.")

# Display DB contents for debugging
display_all_qa()
...
def generate_dynamic_answers(user_question, num_responses=5):
    """
    Generate multiple GPT answers to 'user_question', ensuring variety.
    """
    pastResponses = []
    answers = []
    try:
        for _ in range(num_responses):
            ...
            prompt = (
                f"You are a helpful assistant. Generate one possible answer to this question: {user_question}. "
                f"Do not answer with any of these responses: {pastResponses}. Provide a short, single-sentence answer. "
                f"Notice the past responses and try to make your answer different than the others. "
...
def get_top_responses(user_question, top_n=4, similarity_threshold=0.55):
    """
    Hybrid approach:
    1) Compute cosine similarity with existing Q&A in DB.
    2) Add the best matches if similarity >= threshold.
    3) Use GPT to generate additional candidate answers, pick top by similarity.
    4) Return final top_n.
    """
    global sample_data, sample_questions, sample_embeddings

    # Re-fetch from DB in case new Q&A was added
    sample_data = fetch_all_qa()
    sample_questions = [item["question"] for item in sample_data]
    sample_embeddings = st_model.encode(sample_questions)
    ...
    # Add top matches via cosine similarity
    ...
    if remaining > 0:
        # Use GPT to generate additional responses
        gpt_responses = generate_dynamic_answers(user_question, num_responses=remaining * 2)
        # Filter out error messages
        gpt_responses = [resp for resp in gpt_responses if not resp.startswith("Error")]

        if not gpt_responses:
            return [r["answer"] for r in final_responses], \
                   [r["similarity"] for r in final_responses], \
                   [r["method"] for r in final_responses]

        # Compute similarity for GPT responses
        gpt_embeddings = st_model.encode(gpt_responses)
        gpt_similarities = cosine_similarity(user_embedding, gpt_embeddings)[0]

        gpt_with_similarity = [
            {"answer": resp, "similarity": sim}
            for resp, sim in zip(gpt_responses, gpt_similarities)
        ]
        gpt_with_similarity.sort(key=lambda x: x["similarity"], reverse=True)

        for gpt_resp in gpt_with_similarity:
            if len(final_responses) >= top_n:
                break
            final_responses.append({
                "answer": gpt_resp["answer"],
                "similarity": gpt_resp["similarity"],
                "method": "GPT"
            })
            methods.append("GPT")

    answers = [resp["answer"] for resp in final_responses]
    # If from "cosine_similarity", we already have it,
    # if from GPT, compute it again or just use stored
    similarities = [
        resp["similarity"] if resp["method"] == "cosine_similarity" else
        cosine_similarity(user_embedding, st_model.encode([resp["answer"]]))[0][0]
        for resp in final_responses
    ]
    methods_used = [resp["method"] for resp in final_responses]
...
# ==== GUI WITH ElevenLabs Support START ====
...
# =========== NEW: MULTI-MODEL TRAINING FOR 2D CALIBRATION =========== #
multi_models = None  # will hold { 'SVR_X': ..., 'SVR_Y': ..., 'RF_X': ..., 'RF_Y': ..., 'GBR_X': ..., 'GBR_Y': ..., 'KNN_X': ..., 'KNN_Y': ... }

def train_all_models(calibration_data):
    """
    Trains four regression models (SVR, RF, GBR, KNN) for X and Y
    using the 2D input (normalized_x, normalized_y) from the second code.
    """
    print("[INFO] Training multiple ML models (SVR, RF, GBR, KNN) with 2D input...")
    # Prepare data
    X = np.array([data[0] for data in calibration_data])  # shape (N, 2)
    ...
    # 1. SVR
    svr_x = make_pipeline(StandardScaler(), SVR(kernel='rbf', C=100, gamma='scale', epsilon=0.1))
    svr_y = make_pipeline(StandardScaler(), SVR(kernel='rbf', C=100, gamma='scale', epsilon=0.1))
    svr_x.fit(X, y_x)
    svr_y.fit(X, y_y)
    models['SVR_X'] = svr_x
    models['SVR_Y'] = svr_y
    ...
    print("[INFO] All four models trained successfully.")
    return models


# For drawing multiple blobs, one color per ML technique
model_positions = {
    'SVR': None,
    'RF': None,
    'GBR': None,
...
def handle_question():
    """
    Records/transcribes the user question, then uses semantic approach to get top 4 answers.
    """
    ...
    # Generate 4 answers using new semantic approach
    answers, sims, methods = get_top_responses(context, top_n=4, similarity_threshold=0.55)
...
while cap.isOpened():
    ...
    if phase == "vertical_calibration":
        ...
        if current_calibration_index < len(calibration_points):
            ...
            # ======= TRAIN ALL MODELS HERE AFTER CALIBRATION ======= #
            if len(calibration_data) >= 3:
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                try:
                    ...
                    # ====== Original Single-Blob Approach (RED BLOB) ====== #
                    ...
                    draw_blob(frame, blob_position, blob_radius, (0, 0, 255))  # keep this red

                    # ============ NEW: Multi-Model Predictions ============ #
                    if calibration_complete and multi_models is not None:
                        # Predict each model's screen coordinate
                        X_in = np.array([[normalized_x, normalized_y]])  # shape (1, 2)

                        # SVR
                        svr_x = multi_models['SVR_X'].predict(X_in)[0]
                        svr_y = multi_models['SVR_Y'].predict(X_in)[0]
                        if model_positions['SVR'] is None:
                            model_positions['SVR'] = (int(svr_x), int(svr_y))
                        else:
                            old_pos = model_positions['SVR']
                            new_x = int(old_pos[0] * (1 - smoothing_factor) + svr_x * smoothing_factor)
                            new_y = int(old_pos[1] * (1 - smoothing_factor) + svr_y * smoothing_factor)
                            model_positions['SVR'] = (new_x, new_y)
                    ...
                    # The user's final answer is whichever region they've gazed at long enough
                    # Then we store (context, that answer) in the DB
                    # Then we do the ChatGPT final response reading, etc.
                        ...
                        if calibrated_vertical_direction != opposite_directions["up"]:
                            ...
                            if direction_timer["up"] >= AGREEMENT_THRESHOLD:
                                if len(arrayOfResponses) > 0:
                                    ...
                                    # 1) Add the Q&A to the database
                                    add_qa_to_db(context, selected_response)

                                    # 2) Send the final ChatGPT prompt to form a short sentence
                                    prompt = (
                                    ...
                                    try:
                                        ...
                                        # Check if we need to send emergency email
