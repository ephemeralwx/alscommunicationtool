...
dwell_time = 2  # Dwell time for gaze at each point (in seconds)
start_time = None
buffer_start_time = None
post_calibration_buffer = 0.3  # 300ms after registration
pre_calibration_buffer = 0.3  # 300ms before activation
circle_radius = 50  # Initial circle size for animation
circle_decrement = 1  # How much the circle shrinks per frame

# Blob properties
blob_position = None  # Initial blob position
smoothing_factor = 0.2  # Weight for smoothing blob movement
blob_radius = 50  # Initial blob radius
...
while cap.isOpened():
    ...
    if not calibration_complete:
        if current_calibration_index < len(calibration_points):
            ...
            # Handle pre-calibration buffer
            if buffer_start_time is None:
                buffer_start_time = time.time()

            elapsed_buffer_time = time.time() - buffer_start_time
            if elapsed_buffer_time < pre_calibration_buffer:
                # Show the point but don't register gaze yet
                cv2.circle(frame, (target_x, target_y), circle_radius, (0, 255, 255), 2)
                print(f"[BUFFER] Preparing for calibration at ({target_x}, {target_y})")
                continue

            # Reset animation parameters for the current point
            if elapsed_buffer_time < pre_calibration_buffer + dwell_time:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    if time.time() - start_time > dwell_time:
                        ...
                        # Post-calibration buffer
                        time.sleep(post_calibration_buffer)

                        current_calibration_index += 1
                        buffer_start_time = None  # Reset buffer timer
                        circle_radius = 50  # Reset circle for next point
                        start_time = None
                        break
        else:
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                try:
                    ...
                    print(f"[TRACKING] Gaze mapped to ({screen_x}, {screen_y}) with blob at {blob_position}")
