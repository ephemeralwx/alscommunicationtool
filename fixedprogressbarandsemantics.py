...
def get_top_responses(user_question, top_n=4, similarity_threshold=0.55, diversity_threshold=0.7):
    ...
    # Add top matches via cosine similarity with diversity check
    ...
    if remaining > 0:
        ...
        # Filter out error messages and ensure semantic uniqueness
    ...
    # If still not enough, fill with "N/A"
...
# Modified get_top_responses with semantic uniqueness
def get_top_responses(user_question, top_n=4, similarity_threshold=0.55, diversity_threshold=0.7):
    """
    Hybrid approach:
    1) Compute cosine similarity with existing Q&A in DB.
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                try:
                    ...
                    # Region checks based on the main blob_position
                    # For consistency, you can decide to base this on a primary model or aggregate data
                    # Here, we'll base it on the SVR model as an example
                    if multi_models is not None and model_positions['SVR'] is not None:
                        primary_pos = model_positions['SVR']
                        ...
                        # Fallback to single blob_position if multi_models not available
